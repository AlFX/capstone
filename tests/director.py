...
class DirectorTestCase(unittest.TestCase):
    ''' Test case for the Casting Director role which uses the JWT2 environment
    ...
    def test_movie_1_add_fail(self):
        ''' Test inability to add a movie for the Casting Director'''
        # add
        res = self.client().post(
    ...
    def test_movie_2_update(self):
        ''' Edit the 1st movie title '''
        # get the first movie from the database
    ...
    def test_movie_3_update(self):
        ''' Restore the first movie initial title '''
        # get the first movie from the database
        movie = Movie.query.get(1)
        # update
        res = self.client().patch(
            f"/movies/update/{movie.id}?title=Casablanca",
            headers=[
                ('Content-Type', 'application/json'),
                ('Authorization', f'Bearer {self.jtw}')
            ])
        ...
        # compare with lowercase because of automatic movie title
        # capitalization upon input, see capitalize_all() helper function
        self.assertEqual(movie.title.lower(), 'casablanca')

    def test_movie_4_delete_fail(self):
        # delete
